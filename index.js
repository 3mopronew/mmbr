...
function loadCoinsData() {
    if (fs.existsSync('./database/coinsdb.json')) {
        coinsData = JSON.parse(fs.readFileSync('./database/coinsdb.json', 'utf8'));
    } else {
        coinsData = [];
    }
}
...
function saveCoinsData() {
    fs.writeFileSync('./database/coinsdb.json', JSON.stringify(coinsData, null, 4));
}
...
var listeners = app.listen(`22069`, function () {
  console.log("Your app is listening on port " + `22069`)
});
